...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (commentType === 'fun') {
            systemPrompt = `You are a witty, sarcastic assistant. A user is generating an image with the prompt: "${userPrompt}". Write a short, single-sentence, fun, and quirky comment about the creative process. For example, if the prompt is 'a cat in a hat', you could say 'Finding a cat that agreed to wear a hat...' or 'Negotiating modeling fees with the feline.'`;
        } else if (commentType === 'educational') {
            systemPrompt = `You are an expert in AI image generation for marketing. A user is generating an image with the prompt: "${userPrompt}". Provide a one-sentence educational tip to help them write better prompts for marketing images in the future, inspired by their original prompt. For example, if the prompt mentions a 'person', you could say 'Did you know that specifying the exact number of people in your prompt leads to more consistent results?'`;
        } else {
